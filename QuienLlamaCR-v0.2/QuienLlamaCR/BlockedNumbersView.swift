...
struct BlockedNumbersView: View {
    ...
    var body: some View {
        List {
            if numbers.isEmpty {
                ContentUnavailableView("Sin números bloqueados", systemImage: "phone", description: Text("Los números que bloqueés aparecerán aquí."))
            } else {
        ...
        .toolbar {
            Button("Recargar filtro") {
                Task {
                    do { try await CallDirectoryReloader.reload(); message = "Filtro recargado." }
                    catch { message = "No se pudo recargar. Revisá que la extensión esté activada en iOS." }
                }
            }
        }
    }
